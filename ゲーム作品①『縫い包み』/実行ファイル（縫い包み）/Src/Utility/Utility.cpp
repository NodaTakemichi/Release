...
bool Utility::TimeOver(float& totalTime, const float& waitTime)
{
    //デルタタイム
...
int Utility::GetStringLength(const std::wstring& string, const int& fontHandle)
{
    //名前　文字列
    auto n = string.c_str();
    //文字列の文字数の取得
    int len = static_cast<int>(wcslen(n));
    //文字列の長さを取得
