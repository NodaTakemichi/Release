...
{
	SetScreen();
	auto wstr = StringUtility::StringToWstring(String);
	int rtnFlag = DxLib::DrawString(x, y, wstr.c_str(), Color);
	RevScreen();
	return rtnFlag;
}
...
void _DebugDispOut::TRACE_S(int Color, std::wstring str, int num)
{
	SetScreen();
	auto length = GetDrawFormatStringWidth((str + L":%d").c_str(), num);
	DrawBox(mesPosX_, mesPosY_ + drawLineY_, mesPosX_ + length, mesPosY_ + drawLineY_ + 16, 0, true);
	DrawFormatString(mesPosX_, mesPosY_ + drawLineY_, Color, (str + L":%d").c_str(), num);
	RevScreen();
	drawLineY_ += 16;
}

void _DebugDispOut::TRACE_S(int Color, std::wstring str, double num)
{
	SetScreen();
	auto length = GetDrawFormatStringWidth((str + L":%f").c_str(), num);
	DrawBox(mesPosX_, mesPosY_ + drawLineY_, mesPosX_ + length, mesPosY_ + drawLineY_ + 16, 0, true);
	DrawFormatString(mesPosX_, mesPosY_ + drawLineY_, Color, (str + L":%f").c_str(), num);
	RevScreen();
	drawLineY_ += 16;
}

void _DebugDispOut::TRACE_S(int Color, std::wstring str, std::wstring str2)
{
	SetScreen();
	auto length = GetDrawFormatStringWidth((str + L":%s").c_str(), str2.c_str());
	DrawBox(mesPosX_, mesPosY_ + drawLineY_, mesPosX_ + length, mesPosY_ + drawLineY_ + 16, 0, true);
	DrawFormatString(mesPosX_, mesPosY_ + drawLineY_, Color, (str + L":%s").c_str(), str2.c_str());
	RevScreen();
	drawLineY_ += 16;
}
...
void _DebugDispOut::DrawFPS(void)
{
	...
	_dbgDrawFormatString(4, 4, 0xffffff, L"fps:1/%d", fpsView_);
}
