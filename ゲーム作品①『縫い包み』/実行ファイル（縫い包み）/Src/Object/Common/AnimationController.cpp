...
void AnimationController::Add(int type, const std::string& path, float speed)
{
	...
	auto pathStr = StringUtility::StringToWstring(path);
	anim.model = MV1LoadModel(pathStr.c_str());
	anim.animIndex = type;
	anim.speed = speed;
...
void AnimationController::Play(int type, bool isLoop, 
	float startStep, float endStep, bool isStop, bool isForce, float stepBlend)
{
	...
	if (playType_ != type || isForce) {
		...
		if (playType_ != -1)
		{
			...
			{
				prePlayAnimAttachNum_ = MV1DetachAnim(modelId_, prePlayAnimAttachNum_);
				MV1SetAttachAnimBlendRate(modelId_, playAnim_.attachNo, 1.0f);
			}

			//遷移前アニメーションにセット
			prePlayAnimAttachNum_ = playAnim_.attachNo;
			blendTime_ = 0.0f;
		}
...
void AnimationController::Update(void)
{
	...
	if (!isStop_)
	{
		...
		if (stepBlend_ > blendTime_)
		{
			float rate = blendTime_/ stepBlend_;
			MV1SetAttachAnimBlendRate(modelId_, prePlayAnimAttachNum_, 1.0f - rate);
			MV1SetAttachAnimBlendRate(modelId_, playAnim_.attachNo, rate);

			//ブレンド終了
			if (deltaTime + blendTime_ >= stepBlend_)
			{
				prePlayAnimAttachNum_ = MV1DetachAnim(modelId_, prePlayAnimAttachNum_);
				MV1SetAttachAnimBlendRate(modelId_, playAnim_.attachNo, 1.0f);
			}
