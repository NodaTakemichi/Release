...
static float3 topColor = float3(0.0f, 0.6f, 1.0f);		//青色
static float3 bottomColor = float3(0.0f, 0.9f, 1.0f);	//水色
...
float4 main(PS_INPUT PSInput) : SV_TARGET
{
	...
	float grad = frac(uv.y * verticalSpiltNum);//縦：分割線
	c.rgb *= grad;
